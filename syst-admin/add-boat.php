<?php 
	...
	if(isset($_GET['id'])){
		...
		if(mysqli_num_rows($run) < 1){
			header("Location: services.php?error=snf");
		}
						...
						<form action="" method="post">
							...
							<div class="form-row">
								...
								<div class="form-group col-md-6">
									...
									<input type="text" class="form-control" name="hours" placeholder="000.00">
								</div>
							</div>
							<div class="form-row">
								...
								<div class="form-group col-md-4">
									<label for="size">Engine Size</label>
									<input type="text" class="form-control" name="size" placeholder="Engine Size">
								</div>
								<div class="form-group col-md-4">
							...
							<div class="form-row justify-content-center">
								<div class="form-group">
									<button class="btn btn-success btn-lg" type="submit" name="boat-submit">
										Add Boat
									</button>
								</div>
							</div>
...
<?php
	if(isset($_POST['boat-submit'])){
		...
		$insert = "INSERT INTO boats (customer, make, model, year, color, vin, engine_size) VALUES (". $customer . ",'" .$make. "','" .$model. "'," .$year. ",'" .$color. "','" .$vin. "','" .$size. "')";
		$run = mysqli_query($conn, $insert);
		$select = "SELECT * FROM boats WHERE vin='" . $vin . "'";
		$run = mysqli_query($conn, $select);
		$row = mysqli_fetch_assoc($run);

		$update = "UPDATE services SET boatid=" . $row['boatid'] . " , hours=" . $hours . " WHERE serviceid=" . $service;
